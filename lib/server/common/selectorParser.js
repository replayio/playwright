...
function splitSelectorByFrame(selectorText) {
  ...
  for (let i = 0; i < selector.parts.length; ++i) {
    ...
    if (part.name === 'control' && part.body === 'enter-frame') {
      if (!chunk.parts.length) throw new _selectorErrors.InvalidSelectorError('Selector cannot start with entering frame, select the iframe first');
      result.push(chunk);
      chunk = {
        parts: []
      };
  ...
  if (!chunk.parts.length) throw new _selectorErrors.InvalidSelectorError(`Selector cannot end with entering frame, while parsing selector ${selectorText}`);
  result.push(chunk);
  if (typeof selector.capture === 'number' && typeof result[result.length - 1].capture !== 'number') throw new _selectorErrors.InvalidSelectorError(`Can not capture the selector before diving into the frame. Only use * after the last frame has been selected`);
  return result;
}
...
function parseSelectorString(selector) {
  ...
  const append = () => {
    ...
    if (capture) {
      if (result.capture !== undefined) throw new _selectorErrors.InvalidSelectorError(`Only one of the selectors can capture using * modifier`);
