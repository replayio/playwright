...
class BrowserType extends instrumentation_1.SdkObject {
    ...
    executablePath(options) {
        return this._registry.executablePath(this._name, /* allowReplay */ true) || '';
    }
    name() {
        return this._name;
    }
    ...
    async _launchProcess(progress, options, isPersistent, browserLogsCollector, userDataDir) {
        ...
        if (!(await existsAsync(executable))) {
            ...
            if (!executablePath)
                errorMessageLines.push(`Try re-installing playwright with "npm install @recordreplay/playwright"`);
