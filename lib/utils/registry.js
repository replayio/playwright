...
class Registry {
    ...
    browserDirectory(browserName, allowReplay) {
        ...
        if (allowReplay) {
          // Firefox builds currently require this in order to record content processes.
          ...
          switch (`${process.platform}:${browser.name}`) {
            case "darwin:firefox":
            case "linux:firefox":
            case "linux:chromium":
              return `${replayDir}/playwright`;
            default:
              break;
          }
        }
    ...
    executablePath(browserName, allowReplay) {
        const browserDirectory = this.browserDirectory(browserName, allowReplay);
